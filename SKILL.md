...
### 阶段3.6：任务拆解（Implementation Plan）

把 PRD + Feature List 转成"工程师零上下文就能干"的 **真实代码工程** bite-sized 任务清单，输出 `<产物目录>/task-breakdown.md`。

> **注意**：本段是给真实代码用的，不包括原型 HTML 本身。原型在阶段 4 出，UI 实现任务在 `task-breakdown.md` 里被拆为单条"按 prototype.html 实现组件"。

**优先调用** `Skill('superpowers:writing-plans')`：
- 每步 2-5 分钟（写测试 → 跑失败 → 实现 → 跑通过 → commit，TDD 流可省测试）
- exact 文件路径 + 完整代码块（严禁 TBD / "see Task N" / "add appropriate handling"）
- 内联 3 项 self-review：spec 覆盖 / placeholder 扫描 / 类型一致性
- 写完的 plan 可配套 `Skill('superpowers:subagent-driven-development')` 或 `Skill('superpowers:executing-plans')` 由 subagent / inline 实施

**降级 fallback**（未装 superpowers 时）：照 [references/task-breakdown-fallback.md](references/task-breakdown-fallback.md) 模板手写，TDD 流与非 TDD 流二选一。
...
|------|------|
| 任务标题 | ✅ |
| Files（Create / Modify / Test）| ✅ exact 路径 |
| 步骤 checkbox（5 步 TDD 或 3 步非 TDD）| ✅ |
| 完整代码块 | ✅ 不能跨任务引用 |
...
- `PRD.md` - 完整需求文档
- `task-breakdown.md` - 任务拆解（阶段 3.6 生成）：bite-sized 工程师实施清单
- `design-system/` - 设计系统
...
| `superpowers:writing-plans` | 阶段 3.6 PRD → bite-sized 任务拆解 | 走 [references/task-breakdown-fallback.md](references/task-breakdown-fallback.md) 模板手写 |
